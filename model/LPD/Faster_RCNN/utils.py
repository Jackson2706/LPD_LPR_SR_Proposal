...
def calc_gt_offsets(pos_anc_coords, gt_bbox_mapping, device=device):
    pos_anc_coords = pos_anc_coords.to(device)
    gt_bbox_mapping = gt_bbox_mapping.to(device)
...
def gen_anc_centers(out_size, device=device):
...
def project_bboxes(bboxes, width_scale_factor, height_scale_factor, mode='a2p', device=device):
...
def generate_proposals(anchors, offsets, device=device):
...
def gen_anc_base(anc_pts_x, anc_pts_y, anc_scales, anc_ratios, out_size, device=device):
    anc_pts_x = anc_pts_x.to(device)
    anc_pts_y = anc_pts_y.to(device)

    n_anc_boxes = len(anc_scales) * len(anc_ratios)
    anc_base = torch.zeros(1, anc_pts_x.size(dim=0), anc_pts_y.size(
        # shape - [1, Hmap, Wmap, n_anchor_boxes, 4]
        dim=0), n_anc_boxes, 4, device=device)
        ...
        for jx, yc in enumerate(anc_pts_y):
            ...
            for i, scale in enumerate(anc_scales):
                for j, ratio in enumerate(anc_ratios):
                    ...
                    anc_boxes[c, :] = torch.tensor(
                        [xmin, ymin, xmax, ymax], device=device)
                    c += 1
...
def get_iou_mat(batch_size, anc_boxes_all, gt_bboxes_all, device=device):
    ...
    tot_anc_boxes = anc_boxes_flat.size(1)

    # Create a placeholder to compute IoUs amongst the boxes on the same device
    ious_mat = torch.zeros(
        (batch_size, tot_anc_boxes, gt_bboxes_all.size(1)), device=device)
...
def get_req_anchors(anc_boxes_all, gt_bboxes_all, gt_classes_all, pos_thresh=0.7, neg_thresh=0.2, device=device):
    '''
    ...
    GT_bboxes = torch.gather(
        gt_bboxes_expand, -2, max_iou_per_anc_ind.reshape(B, tot_anc_boxes, 1, 1).repeat(1, 1, 1, 4))
    ...
    negative_anc_ind = negative_anc_ind[torch.randint(
        0, negative_anc_ind.shape[0], (positive_anc_ind.shape[0],))].to(device)
